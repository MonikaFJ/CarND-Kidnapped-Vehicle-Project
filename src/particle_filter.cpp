...
//      observation.y = particle.y + vec*cos(particle.theta); //TODO if correct?
      ...
      obs.x = particle.x + obs.x * cos_theta - obs.y * sin_theta;
      obs.y = particle.y + obs.x * sin_theta + obs.y * cos_theta;
    }
    return observations;
  };
...
//    return distance_xy;
  ...
  for (auto particle : particles)
  {
    ...
    for (auto obs : observations_in_map){
      ...
      particle.weight = particle.weight * multiv_gauss.multiv_prob(std::get<0>(dist),std::get<1>(dist));
    }
